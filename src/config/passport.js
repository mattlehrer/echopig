      ...
      (req, accessToken, tokenSecret, profile, done) => {
        if (req.user) {
          User.findOne({ twitter: profile.id }, (err, existingUser) => {
            ...
            if (existingUser) {
              logger.info(
                `Logged in: ${req.user.username} - Twitter account: ${profile} registered to different user: ${existingUser.username}`,
              );
              req.flash(
                'errors',
                ...
                (err, existingEmail) => {
                  ...
                  if (existingEmail) {
                    ...
                    return createUser(newUser, (err, user) => {
                      ...
                      logger.info(
                        `Created user on Twitter login: ${JSON.stringify(
                          user,
                        )}`,
                      );
                      return done(null, user);
      ...
      (req, accessToken, refreshToken, profile, done) => {
        if (req.user) {
          User.findOne({ facebook: profile.id }, (err, existingUser) => {
            ...
            if (existingUser) {
              logger.info(
                `Logged in: ${req.user.username} - Facebook account: ${profile} registered to different user: ${existingUser.username}`,
              );
              req.flash(
                'errors',
